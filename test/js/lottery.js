function lotteryModule(c){
	var c = c || {};
	var defaultConfig = {
		lotteryBox: '#lotteryBox', //整个转盘id选择器
		giftItem: '.giftItem', //单个奖品选择器
		sort: [], //排序，默认为顺序
		targetClass: 'on', //选中时的class，默认on
		fastSpeed: 80, //快速阶段的速度(定时器时间间隔)，默认80
		slowSpeed: 400, //慢速阶段的速度(定时器时间间隔)，默认400
		startStep: 4, //开头的慢速阶段跑几步，默认4
		endStep: 4 //结束的慢速阶段跑几步，默认4
	};
	var config = {};

	var timeIntervalId; //定时器
	var isRuning; //是否正在抽奖中
	var curStatus; //0:停止  1:启动截断 2:中间快速阶段  3:中奖减速阶段
	var giftIndex; //抽中奖品的位置,-1表示还没获得
	var curIndex; //当前在哪个位置
	var runCount; //累计总共跑了几个位置
	var endCallback; //用来注册结束时回调的函数
	function reset(){
		config = $.extend(defaultConfig, c);
		timeIntervalId = 0; //定时器
		isRuning = false; //是否正在抽奖中
		curStatus = 0; //0:停止  1:启动截断 2:中间快速阶段  3:中奖减速阶段
		giftIndex = -1; //抽中奖品的位置,-1表示还没获得
		curIndex = 0; //当前在哪个位置
		runCount = 0; //累计总共跑了几个位置
		endCallback = null; //用来注册结束时回调的函数
	...
	var giftCount = $lotteryBox.find(config.giftItem).length; //奖品总数
	...
	function stepFunction(){	
		...
		if(curStatus == 2 && (runCount >= (config.startStep + giftCount))){ //至少要快速跑一圈
			if(giftIndex != -1){
